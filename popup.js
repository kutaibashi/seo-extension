...
function createLinkTable(linksArray, containerId, captionKey) {
    ...
    linksArray.forEach(link => { const row = tbody.insertRow(); row.classList.add('clickable-item'); row.dataset.tagName = link.tagName; row.dataset.elementIndex = link.elementIndex; row.dataset.linkUrl = link.href; const linkTextHtml = link.text ? safeText(link.text) : (translations[currentLang].linksNoText || '<i>(No text / Image link)</i>'); const linkUrlHtml = link.href ? `<a href="${safeUrl(link.href)}" title="${safeText(link.href)}" target="_blank" rel="noopener noreferrer">${safeText(link.href)}</a>` : 'N/A'; row.innerHTML = `<td>${linkTextHtml}</td><td>${linkUrlHtml}</td><td>${link.nofollow ? 'Yes' : 'No'}</td><td class="link-status-code">${translations[currentLang].crawlStatusLoading || '...'}</td>`; });
