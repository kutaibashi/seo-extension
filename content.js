...
(function () {
    ...
    function getContentAnalysis() { console.log('[SEO Analyzer - content.js] Getting Content Analysis...'); let textContent = ''; let wordCount = 0; let letterCount = 0; const mainSelectors = ['main', 'article', '[role="main"]', '.main-content', '#main-content', '.post-content', '#content', '.entry-content']; let mainElement = null; for (const selector of mainSelectors) { mainElement = document.querySelector(selector); if (mainElement) break; } if (!mainElement) mainElement = document.body; if (mainElement) { try { const clone = mainElement.cloneNode(true); clone.querySelectorAll('header, footer, nav, aside, script, style, noscript, iframe, svg, button, form, input, textarea, select, label, img, figure, .advertisement, .ads, .sidebar, .comments, .related-posts, .share-buttons, .breadcrumbs, .pagination, .widget, .footer-widget, .header-widget, [aria-hidden="true"], link, meta').forEach(el => el.remove()); textContent = clone.textContent || ''; textContent = textContent.replace(/\s+/g, ' ').trim(); if (textContent) { letterCount = textContent.length; wordCount = textContent.split(/\s+/).filter(word => word.length > 0).length; } } catch (e) { console.error("Error during content analysis cloning/cleanup:", e); textContent = mainElement.textContent || ''; textContent = textContent.replace(/\s+/g, ' ').trim(); if (textContent) { letterCount = textContent.length; wordCount = textContent.split(/\s+/).filter(word => word.length > 0).length; } } } return { wordCount, letterCount }; }
